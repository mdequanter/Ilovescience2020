...
while True:
    ...
    if (difference > 0.5 ) :
        if (counter >=1) :
            ...
            print ("driver " + str(counter) + ":" + str(firstPulseTime))
            ...
            lapTime = datetime.strptime(firstPulseTimeString, FMT) - datetime.strptime(lastTimeString, FMT)
            lastTime[driver] = datetime.now()
            
            difference = diff.seconds + diff.microseconds / 1000000
            cursor = conn.cursor()
            cursor.execute("INSERT INTO laps (tag,drivername,lap,laptime,timestamp) VALUES (?,?,?,?,?)",
                           (network_id,drivername,lapCounter[driver],lapTime, str(datetime.now())))
            conn.commit()
